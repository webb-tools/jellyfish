...
pub struct Commitment<F, H>
where
    F: ParameterHelper,
    H: PermutationTrait<F>,
{
    hash: H,
    input_len: usize,
    phanton: PhantomData<F>,
}

impl<F, H> CommitVerify<F> for Commitment<F, H>
where
    F: ParameterHelper,
    H: PermutationTrait<F>,
{
    /// Create a new commitment instance for inputs of length `input_len`
    fn new(input_len: usize) -> Commitment<F, H> {
    ...
    fn commit(&self, input: &[F], blind: &F) -> Result<F, PrimitivesError> {
        if input.len() != self.input_len {
            return Err(PrimitivesError::ParameterError(format!(
                "Commitment error: expected input length ({}). It must Mch \
        ...
        let result_vec = self.hash.hash(msg.as_slice(), 1, RATE)?;
        Ok(result_vec.into_vec()[0])
    }
